...
- `@namzu/browser`, `@namzu/computer-use`, `@namzu/files`, `@namzu/live`, `@namzu/lsp`, `@namzu/sandbox` — optional capability packages
...
| External-name audit | `node --import tsx --test scripts/__tests__/audit-external-names.test.ts scripts/__tests__/check-downstream-names.test.ts && node scripts/audit-external-names.mjs` |
...
**A merge whose tree CI already passed skips that inline validation.** `ci.yml`'s last job, `Record the validated tree`, runs only when both matrix legs and `Docs` succeeded, and uploads an artifact named `validated-tree-<tree sha>` for the tree the gates checked out. `release.yml` first asks `.github/scripts/find-validated-tree.mjs` about `git rev-parse HEAD^{tree}`; it answers yes only for such an artifact, unexpired, from a `ci.yml` run that concluded `success` on `pull_request` or `merge_group` in this repository, never a fork, and that provably executed the pushed commit's own `ci.yml`. The artifact also carries the run's workflow OIDC token (audience `namzu-validated-tree`); the helper verifies its signature against GitHub's published keys, requires its claims to name this repository, this run, this event, `ci.yml` and, for `pull_request`, base `main`, and then requires the `ci.yml` blob at the token's `workflow_sha` to equal `git rev-parse HEAD:.github/workflows/ci.yml` and the tree of its `sha` to be the tree being released. The artifact's name is written by whichever `ci.yml` ran; a `pull_request` run executes the file in `refs/pull/N/merge`, which the PR's base branch can supply, and `main`'s ruleset requires a review but no status check, so without that proof any collaborator's throwaway PR into a branch carrying a forged `ci.yml` could forge the record and turn every release gate off. Neither the run's head commit nor its `pull_requests` (only PRs open now, so empty once a PR merged) says which file ran. Every other outcome, API errors included, is no. On a yes each validation step is skipped through `if: steps.revalidation.outputs.skip != 'true'`. Install and Build always run, the External-name audit always runs too (a `ci.yml` run for a pull request from a fork gets no secrets, so it skipped the downstream-name rule yet still recorded its tree), and the pre-publish consumer install check keeps its version-commit condition either way. A PR merged after `main` moved lands a different tree and gets the full validation, and so does a direct push, unless its tree is byte-identical to one CI passed. The step log and a notice name the path taken and the CI run relied on. The parity check also compares conditions: a `release.yml` gate carries no `if:` or exactly that guard, a guarded step must be a gate `ci.yml` runs, Install, Build and the pre-publish check never carry the guard, a `ci.yml` gate carries no `if:` or exactly `matrix.gates`, and `matrix.gates` only in a job whose own `strategy.matrix.include` has an entry setting `gates: true` (in any other job it is empty and the step is skipped, which is green), no `ci.yml` job that runs a gate carries a job-level `if:` (a skipped job does not fail the run), and no gate on either path, nor the job that runs it, carries `continue-on-error` other than a literal `false` — under it a `ci.yml` gate can fail while the run concludes `success`, the record is written, and `release.yml` skips that gate too. `Record the validated tree` must also list every job that runs a gate in its `needs`, and run under exactly `success() && (github.event_name == 'pull_request' || github.event_name == 'merge_group')`, so that a failed, cancelled or skipped gate job leaves no record; `!failure()` or `always()` would write one over a skipped job.
...
A **separate workflow**, `PR text` (`.github/workflows/pr-text.yml`), runs one job, `Downstream names`, on a pull request into `main` (`edited` included, since a squash merge takes the title and body as they stand) and on `merge_group`. It reads what the final tree does not hold: the title, the body, the head branch's name, and each commit's message, author, committer and added lines and paths, one commit at a time, because a rebase merge lands every commit as it is and `refs/pull/<n>/head` keeps them all reachable. A name one commit adds is refused even when a later commit removes it; the fix is to rewrite the commit that added it. `Record the validated tree` does not wait for it, and `release.yml` has no counterpart, because a direct push carries no pull-request text.

| PR text step | Run it locally |
|---|---|
| No downstream name in the PR text, commits or added lines | `gh api 'repos/{owner}/{repo}/pulls/<N>' --jq '{pull_request: .}' > /tmp/pr-event.json && node scripts/check-downstream-names.mjs --event /tmp/pr-event.json` |

Its tests run in the External-name audit row above.

**The downstream-name list** is the repository secret `NAMZU_DOWNSTREAM_NAMES`, and nothing else. Both checks — the second rule of the External-name audit and `PR text` — read it from the environment variable of that name, which `ci.yml`, `release.yml` and `pr-text.yml` pass to the one step each that runs a check. No file of this repository holds a name in any form, digests included: names are short and guessable, and a committed list of digests was reversed in seconds. Never write a name into a file, a commit message, a PR, a test or a log; tests use placeholders such as `acmecorp` through the variable. The owner maintains the secret with `gh secret set NAMZU_DOWNSTREAM_NAMES`, which prompts for the value (names separated by commas), or with `gh secret set NAMZU_DOWNSTREAM_NAMES < names.txt` from a file kept outside every checkout, one name per line; `--body` would leave the list in the shell's history. A secret cannot be read back, and each `set` replaces the whole list, so the owner keeps the list privately and sets all of it on every change. An entry is a name as written (`Some Name` also matches `some-name`, `SomeName`, `SOME_NAME` and a name inside an identifier such as `someNameHost`), at least four characters once folded and at most four parts. A one-word name that is also an ordinary word may end in `=exact` (`word=exact`): it then matches only a whole word, in any case, never inside `wordHost`, `WORD_TLS` or `words`; a whole word in prose still matches. An entry the checks cannot use fails them, named by its position, never its text.

Without the variable, both checks fail with `NAMZU_DOWNSTREAM_NAMES is not set`: in CI on a pull request from a branch of this repository, a merge group, a push or a release, and locally. Two exceptions. A pull request from a fork gets no secrets, so there both checks skip with a notice that says so; `release.yml`'s External-name audit carries no skip guard and reads the tree with the list on every push to `main`, before anything is published. And locally, a contributor without access runs the audit as `node scripts/audit-external-names.mjs --without-downstream-names`, which runs everything else and says the downstream-name rule did not run (`check-downstream-names.mjs` takes the same flag, and then checks nothing); the flag is refused in GitHub Actions. Nothing either check prints repeats a name: every word carrying one reads `***`, and a line that would still show any entry's text is withheld whole.
