...
Namespaced tool names are provider-valid and injective by construction. The plugin namespace separator is `__`, and components are validated so none may contain it (`read_file` stays legal). Names the plugin author controls — the plugin name, its own tools, its MCP server aliases — are validated strictly, because their author can fix them. Names supplied by a third party — an MCP server's own tool names, connector methods — are canonicalized onto `[a-zA-Z0-9_-]{1,64}` instead; the server is still invoked under its original name. A single nonconforming remote tool (`notion.search`, `db:query`) is repaired, or at worst skipped with a `plugin_tool_skipped` event, and never fails the whole plugin enable.

Canonicalization does not lose the distinction between two remote names. Any repair — a substituted character, a collapsed `__`, a truncation — appends a 7-character hash of the ORIGINAL name, so `a.b`, `a/b` and `a__b` map to three different keys rather than collapsing onto `a_b`. The mapping is a pure function of the raw name, independent of the order MCP servers were enumerated in, so a canonical name persisted in a run history resolves to the same remote tool after a restart. A name that needs no repair is returned unchanged. A canonical name that is nevertheless already taken is skipped and reported, never silently aliased onto the tool that registered first.
