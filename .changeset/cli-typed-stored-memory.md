...
- **Old notes move only when you ask.** The project's curated `MEMORY.md` is not rewritten at launch: nothing can tell a bullet `#note` appended from one you wrote. When it holds single-line bullets, the launch says how many, once per curated file, and `/memory import-notes` moves them into typed memories; the file's text before each run is kept as `MEMORY.md.before-typed-memory` (then `-2`, `-3` for a later run's different text), and a list starting directly under a heading, a bullet followed by a non-bullet line, prose and nested bullets stay. A blank line ends a heading's list, so notes `#note` appended below a heading and a blank line are offered. Until then they are curated text in every turn, as before.
