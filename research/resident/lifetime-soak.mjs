...
assert.ok(checkout?.startsWith('/'), 'Pass the absolute path to a fixed built checkout.')
const intervalMs = Number(process.env.NAMZU_SOAK_INTERVAL_MS ?? 600_000)
assert.ok(Number.isSafeInteger(intervalMs) && intervalMs >= 1000)
const root = await mkdtemp(join(tmpdir(), 'namzu-lifetime-soak-'))
	...
	Object.fromEntries(
		await Promise.all(
			files.map(async (path) => [
				path,
				createHash('sha256')
					.update(await readFile(join(checkout, 'packages', path)))
					.digest('hex'),
			]),
		),
	)
...
ProviderRegistry.create = () => {
 ...
 provider.chatStream = async function* (params) {
  ...
  yield* new MockLLMProvider({turns:[{text:JSON.stringify(answer),usage:{promptTokens:100,completionTokens:20,totalTokens:120,cachedTokens:40,cacheWriteTokens:10}}]}).chatStream(params);
 };
 return {provider};
};
`,
...
try {
	...
	for (let wave = 1; wave <= 12; wave++) {
		...
		const before = (await readFile(requestsPath, 'utf8')).trim().split('\n').length
		await sleep(intervalMs)
		const after = (await readFile(requestsPath, 'utf8')).trim().split('\n').length
		report.idleObservations.push({ at: Date.now(), wave, before, after, intervalMs })
		assert.equal(after, before, 'Idle host must not call the model.')
		...
		if (wave === 8) {
			await setControl('hang', 80)
			await command(['wake', ids[0], 'wave-crash: scripted read-only observation.'])
			state = await until(
				'admitted before abrupt death',
				(s) => s.agenda.pursuits[0].state.phase === 'running',
			)
			...
			const owner = state.runner.owner
			assert.ok(Number.isSafeInteger(owner.pid) && owner.pid > 1)
			process.kill(owner.pid, 'SIGKILL')
			await sleep(500)
			await command(['pause'])
			const crash = state.agenda.pursuits[0].state
			report.abruptExit = {
				pid: owner.pid,
				claimId: crash.claimId,
				revision: crash.revision,
				at: Date.now(),
			}
			await command(['release', owner.instanceId, '--executor-stopped'])
			await command([
				'reconcile',
				ids[0],
				'Inspected isolated scripted provider killed while sleeping. No mutations; missing usage is unknown.',
				'--claim',
				crash.claimId,
				'--revision',
				String(crash.revision),
				'--outcome',
				'wait',
				'--executor-stopped',
			])
			await setControl('wait', 8)
			await command(['resume'])
			await start()
	...
	const deadline = Date.now() + 10_000
	while (!(await readFile(requestsPath, 'utf8')).includes('"epoch":90')) {
		assert.ok(Date.now() < deadline)
		await sleep(100)
	}
	await command(['stop'])
	...
	assert.equal(report.workerPids.length, 4)
	assert.equal(report.finishes.length, 29)
	assert.equal(new Set(report.finishes.map((f) => f.claimId)).size, 29)
	assert.ok(!report.finishes.some((f) => f.claimId === report.abruptExit.claimId))
	...
	for (let wave = 1; wave <= 12; wave++)
		assert.ok(
			report.requests
				.filter((r) => r.control.epoch === wave && r.control.action === 'wait')
	...
	assert.equal(report.providerTokens, 28 * 120)
	for (const id of ids) await command(['archive', id])
	report.archived = await command(['status'])
	assert.equal(report.archived.agenda.pursuits.length, 0)
	report.passed = true
} catch (error) {
