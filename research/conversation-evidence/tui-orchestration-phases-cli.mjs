...
// picker, submit the prompt, approve each launch that is asked about, let two background
// children name a colour, wait for both, run one join child, answer.
//
// usage: node tui-orchestration-phases-cli.mjs <repo-root> <label> <cols> <rows>
// Writes <label>.txt into the run's own temp root: every checkpoint rendered through a
// fresh headless VT emulator, then the whole buffer. The frames in
// tui-orchestration-phases-results.md came from feat/orchestration-tui-parity at
// 0a394539 (before) and with read-only launches unasked and phases on the rail (after).
//
// Phase 1 is two read-only (explore) agents; Phase 2 is a general-purpose agent,
// which can write. A build that reviews every launch asks twice; one that lets
// read-only launches start asks once. The `key_if` step answers either.
	...
	switch (assistants) {
		...
		case 2:
			return { usage, text: 'Phase 1 returned Blue and Green; starting Phase 2.', toolCalls: [
				{ id: 'j1', name: 'Agent', args: { description: 'Join the two colours', prompt: 'JOIN: make one short sentence from Blue and Green.', workflow: WORKFLOW, phase: 'Phase 2', phase_order: 1 } },
			] }
		default:
...
function delayFor(params) {
	...
	if (s.includes('JOIN:')) return 4000
...
const STEPS = [
	...
	// Phase 1 is two read-only (explore) agents in one response. A build that
	// reviews every launch asks "Start 2 agents"; one that exempts read-only
	// launches starts them. Either way their names reach the screen.
	{ op: 'type_submit', text: PROMPT, expect: 'Choose second colour', timeout: 40, label: 'prompt' },
	{ op: 'checkpoint', name: 'phase1_start', settle: 0.5 },
	{ op: 'key_if', pattern: 'Start 2 agents', name: 'y', label: 'phase1_was_reviewed', settle: 1.4 },
	{ op: 'checkpoint', name: 'phase1_running' },
	{ op: 'sleep', seconds: 1.4 },
	{ op: 'checkpoint', name: 'phase1_one_done' },
	// Phase 2 is a general-purpose agent, which can write: asked in both builds.
	...
	{ op: 'key', name: 'ctrl_t', settle: 0.8 },
	{ op: 'checkpoint', name: 'cockpit_phase2_running' },
	{ op: 'key', name: 'escape', settle: 0.6 },
	{ op: 'wait_count', pattern: 'sit side by side', count: 2, timeout: 40, label: 'final' },
	{ op: 'sleep', seconds: 2.0 },
	{ op: 'checkpoint', name: 'turn_end' },
	{ op: 'key', name: 'ctrl_t', settle: 0.8 },
	{ op: 'checkpoint', name: 'cockpit_done' },
	{ op: 'key', name: 'right', settle: 0.5 },
	{ op: 'key', name: 'enter', settle: 0.8 },
	{ op: 'checkpoint', name: 'drill_in' },
	{ op: 'key', name: 'escape', settle: 0.5 },
	{ op: 'key', name: 'escape', settle: 0.5 },
	{ op: 'checkpoint', name: 'closed' },
]
