...
const root = await mkdtemp(join(tmpdir(), 'namzu-summary-evidence-cli-'));
...
const summaries = Array.from({length:4}, (_,i)=>buildCompactionMessage(`ORCHID original receipt code: not recorded in summary ${i}.`));
...
const paths = ['packages/sdk/dist/run/evidence-recall.js', 'packages/sdk/dist/store/evidence/disk.js', 'packages/sdk/dist/compaction/summary.js', 'packages/sdk/dist/store/evidence/index-page.js', 'packages/sdk/dist/store/evidence/compaction-text.js', 'packages/cli/dist/integrations/sessions/evidence-recall.js', 'packages/cli/dist/integrations/sessions/conversation-search.js'];
...
const report = { root, live, expectMissing, source: { runId, seq: targetSeq, code, original, summaries, archiveIds }, before: await fingerprints(), passed: false };
try {
