...
const root=await mkdtemp(join(tmpdir(),'namzu-token-discovery-cli-'));
...
const events=[{type:'run_started',runId:sourceRunId,seq:1},
 ...Array.from({length:12},(_,i)=>({type:'tool_completed',runId:sourceRunId,seq:i+2,toolName:'read',toolUseId:`note-${i}`,result:`Packing information for unrelated entry ${i}.`,isError:false})),
...
const report={root,live,provider:live?'codex':'scripted',model:live?'gpt-5.6-luna':'scripted',effort:'low',sourceRunId,sessionId,tracking,destination};
...
try{
 ...
 assert.equal(report.discovery.literal.some(p=>p.hasCodes),false);assert.equal(report.discovery.token.some(p=>p.hasCodes),true);
