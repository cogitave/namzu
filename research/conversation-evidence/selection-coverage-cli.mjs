...
const root=await mkdtemp(join(tmpdir(),'namzu-selection-coverage-cli-'));
...
const {createUserMessage,createEvidenceRecallStep}=await import(sdkURL);
...
const events=[{type:'run_started',runId:sourceRunId,seq:1},
 ...Array.from({length:4},(_,i)=>({type:'tool_completed',runId:sourceRunId,seq:i+2,toolName:'read',toolUseId:`note-${i}`,result:`DELTA receipt: tracking code TRACK-old-${i}; destination DEPOT-old-${i}.`,isError:false})),
 {type:'tool_completed',runId:sourceRunId,seq:6,toolName:'read',toolUseId:'original',result:retained.output,isError:false,outputTruncated:true,outputSpillIntegrity:retained.spillIntegrity}];
...
try{
 ...
 const query='What were the tracking code and destination in the last DELTA receipt?';
 let terms;
...
let turn;
if(${JSON.stringify(expectMiss)}) turn={text:context};
else if(step===0){
const address=metadata.additionalEvidence?.[0];
if(!address) throw new Error('No omitted evidence address reached the model request.');
turn={toolCalls:[{id:'read-omitted',name:'read_conversation',args:address}]};
}else turn={text:params.messages.filter(m=>m.role==='tool'&&m.toolCallId==='read-omitted').map(m=>m.content).join('\\n')};
yield* new MockLLMProvider({turns:[turn]}).chatStream(params);
};return created;};`);
 const prompt=query;
 const args=['--quiet','--format','json','run','--trust','--cwd',cwd,'--resume',sessionId,'--provider','codex','--model','gpt-5.6-luna','--effort','low','--max-iterations','4','--token-budget','30000',prompt];
 report.command=args;
 ...
 assert.equal(first.context.includes(tracking)||first.context.includes(destination),false);
 assert.equal(first.metadata.incomplete,false);
 assert.equal(first.metadata.omittedPassages,expectMiss?undefined:1);
 if(!expectMiss) assert.equal(first.metadata.additionalEvidence?.length,1);
 ...
 if(expectMiss) assert.equal(report.calls.length,0);
 else {assert.ok(report.calls.length>0);assert.ok(report.calls.every(call=>call.name==='read_conversation'));}
