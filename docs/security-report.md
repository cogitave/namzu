...
description: Source-based security assessment of Namzu at f58a086f — sixteen findings, a threat mapping, a risk register and a prioritized remediation roadmap, with a status delta re-verified against a later main.
resource: packages/sdk/src/index.ts
tags: [security, architecture, risk, operations]
status: draft
generated: { by: human:bahadirarda, at: 2026-08-11T00:00:00Z }
---
...
Added 2026-08-13, when this report was committed, and re-verified on 2026-09-18 against `main` at `c17993af`. **Everything below assesses `f58a086f`, and `main` left it long ago.** The findings are left exactly as written, because a dated assessment that gets edited to match later code stops being a record of what was true; this section carries the delta instead.

Each line was re-checked by confirming on `main` that the named file exists and that the code still does what the line claims — not by re-testing the finding or re-running the assessment. `Landed` means the half named in the evidence is closed. `Partly` means one half of the finding is discharged and the other is not, and the half that remains is named alongside the issue that tracks it where there is one. The table is not a claim that everything else moved.

| ID | Status | Evidence on `main` |
| --- | --- | --- |
| NAMZU-SEC-001 | Partly | Model control over the destination is gone: `requireSameOrigin` (`packages/sdk/src/connector/http-operation.ts`) is applied to the resolved request URL in `packages/sdk/src/connector/builtins/http.ts` and to the webhook target in `packages/sdk/src/connector/builtins/webhook.ts`, and neither follows a redirect. The finding's other half is unchanged: a bridged connector tool still declares `isDestructive: () => false` whatever method it performs (`packages/sdk/src/connector/tools/adapter.ts`), so a state-changing call is still labelled non-destructive. No issue tracks that half. |
| NAMZU-SEC-002 | Partly | The connector path is tenant-bound: `connectWithCredential` resolves through `retrieveForScope(tenantId, connectorId, credentialId)` and refuses an unavailable reference (`packages/sdk/src/manager/connector/tenant.ts`), and `retrieveForScope` and `revokeForTenant` verify ownership (`packages/sdk/src/vault/InMemoryCredentialVault.ts`). The unscoped `retrieve` and `revoke` are still on `CredentialVault`, documented as host-authority methods (`packages/sdk/src/types/connector/tenant.ts`), so a host that resolves a tenant-supplied reference itself can still cross tenants — the deployment condition the finding already names as raising its risk. |
| NAMZU-SEC-003 | Partly | A server no longer decides its own approval: `isTrustedReadOnly` (`packages/sdk/src/tools/trusted-read-only.ts`) requires per-server operator trust, recorded as `provenance.readOnlyHintTrusted`, before any consumer acts on the claim. The wiring has grown a door rather than moved: four consumers read the predicate today — the kernel's `allow_read_only` rule (`packages/sdk/src/authorization/rules.ts`), the review exemption (`packages/sdk/src/runtime/query/review-policy.ts`, which the CLI re-exports as `isPromptExempt`), the registry's plan-mode block (`packages/sdk/src/registry/tool/execute.ts`) and the read-only roster a delegate is built from (`packages/sdk/src/tools/roster.ts`) — and the CLI's prompt exemption is the kernel's rule rather than a list of its own. The finding's other half is unchanged: with no local admission policy every advertised tool is still accepted, and drift is counted rather than blocked — `applyToolPolicy` admits everything when neither `allow` nor `deny` is set, and `hasDrift` only tallies (`packages/sdk/src/connector/mcp/policy.ts`). No issue tracks that half. |
...
Re-checked in the same pass and unchanged: **NAMZU-SEC-009** (`packages/sandbox/worker/Dockerfile` still installs `xlsx@0.18.5`), **NAMZU-SEC-010** (`sandbox.requireIsolation` still defaults to empty, and `packages/cli/src/config/schema.ts` still gives the reason) and **NAMZU-SEC-011** (workflow actions are still referenced by mutable major tags, `actions/checkout@v5` among them). **NAMZU-SEC-015** is unchanged too: `LocalFsBlobStore` validates a key on `put` and joins the root with it unvalidated on `get`, `head` and `delete` (`packages/files/src/local/index.ts`).

**The remaining findings were not re-assessed in this pass.** NAMZU-SEC-012, NAMZU-SEC-013 and NAMZU-SEC-016 are absent from the table because nobody checked them on 2026-09-18, which is neither open nor closed. Treat the three findings absent from the table as carrying their original status until someone re-runs the assessment.
...
- [Review policy](sdk/review-policy.md) - which calls run without asking, and why a connected server does not get to decide that for its own tools.
