...
`unattendedSandboxEscape: 'allow'` confirms only the sandbox escape. An outside-root path or an unknown program in the same call or batch still needs a person; without a prompt, the whole batch is refused with that boundary's reason.
