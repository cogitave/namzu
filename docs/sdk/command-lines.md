...
Nor can a pattern match a program name that never appears as such anywhere in the line's text: `^git push` (or the unanchored `deny` form) does not deny `$(echo git) push origin main`, `` `echo rm` -rf x`` or `$X push` (`X=git`) — reading the line correctly is not the same as knowing what a substitution or a variable will produce. This is not a gap left for a `deny` rule to close; a call whose command's own program-name word is such an expansion is escalated instead (`ToolCallSummary.escalation.unknownProgram`, see [Crossing the tool boundary](escalations.md)): no allow rule, remembered grant or unattended mode approves it, though a `deny` that matches some other way (the tool's own name, or an unrelated word) still refuses it outright.
