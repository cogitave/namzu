...
It is a lexer, not an interpreter. It reads `env git push`, `command git push`, `xargs sh -c` and `sudo systemctl stop x` as ordinary words — it does not know that `env`, `sudo`, `nice`, `timeout` and the rest re-exec their trailing argv, or that `xargs`' argument names a program of its own — and a `deny` pattern meant to catch a program run through one of them still has to name the wrapper too (`^(sudo |env )?git push`), because `decomposeCommandLine`/`decodedCommands` do not unwrap it. The `sh` dialect's agreement with `dash` is by construction, not by measurement: `dash` is not installed where the differential check runs, so what makes a line transparent in `sh` is only what POSIX and bash agree on.

Nor can a pattern match a program name that never appears as such anywhere in the line's text: `^git push` (or the unanchored `deny` form) does not deny `$(echo git) push origin main`, `` `echo rm` -rf x``, `$X push` (`X=git`) or `env $(echo git) push` — reading the line correctly is not the same as knowing what a substitution or a variable will produce, and unwrapping `env` does not make its argument any less unknown. This is not a gap left for a `deny` rule to close; a call whose command's own program-name word is such an expansion — in the head, or in the position a re-exec wrapper's own real option grammar puts it — is escalated instead (`ToolCallSummary.escalation.unknownProgram`, see [Crossing the tool boundary](escalations.md)): no allow rule, remembered grant or unattended mode approves it, though a `deny` that matches some other way (the tool's own name, or an unrelated word) still refuses it outright. `programPositions`/`resolveScriptPrograms` (`packages/sdk/src/authorization/program.ts`) is where that unwrapping lives — a second, narrower reader built on top of this one, for exactly one question ("where is the program named") rather than "what commands does this line run", which is why it can afford to understand `env`/`sudo`/`nice`/`timeout`/`xargs`'s own grammars where the lexer itself does not.
