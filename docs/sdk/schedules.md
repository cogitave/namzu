...
- `create` needs `name`, `when` and `permissions`, plus `prompt` for `agent`/`script+agent` or `script` for `script`/`script+agent`; `permissions` needs `unmatched` plus a preset or rules. There is no default permission set.
...
- The host computes the preview (`ScheduleJobPreview`): the canonical folder, the schedule in words, the next fire times, the expanded rules, the effective budget, and the agent phase's model when one exists. For a pure script, `model` and `dailyTokenCeiling` are absent, `budget.maxIterations` and `budget.tokenBudget` are zero, and `budget.timeoutMs` is the script's own timeout. The tool refuses a proposed `budget` on a pure script; use `script.timeoutMs` to limit it. A successful pure script create reports its history and that it opens no session. `networkAccess` reports whether the run can reach the network, including through a host script or an allowed host shell; the new optional `networkGrantAccess` reports whether stored web/browser permissions grant it. Hosts can omit `networkGrantAccess`; the update guard then conservatively reads `networkAccess`. The model's words are never shown as fact. The input schema describes `budget` as the agent phase's limits and `maxIterations` as model steps, not repetitions: a model once proposed `maxIterations: 1` for a job meant to post once per run, and its first run stopped after one model call. `tokenBudget` is the whole agent phase's, with every model call resending the prompt; the next proposal set 4 000 tokens for runs that each took about 110 000.
...
- `update` changes an existing job in place: `job` names it, and only the fields the model sets (`prompt`, `when`, `folder`, `tz`, `budget`, `kind`, `script`, or `permissions` as the whole new set, under the same limits as `create`) are passed on, as `ScheduleJobChanges`; `name` is refused (a job keeps its name). `kind`/`script` are checked the same way `create`'s are (`kind: 'agent'` given together with `script` is refused, and the effective preview is checked against the network-beside-a-host-shell rule even when a kind-only change inherits existing permissions); a host that leaves both unset in `ScheduleJobChanges` keeps the job's current kind and script, exactly as before this field existed. The host's optional `previewUpdate(job, changes)` computes `ScheduleJobUpdateProposal` — the job as it would run (`preview`), the `-`/`+` lines of what differs (`changes`) and whether what a run may do changes (`permissionsChange`) — or throws with a refusal (which is how a host re-verifies a proposed script against the scheduled-run floor and deny rules, the same as `create`'s); `confirmUpdate(request, signal)` asks the person (anything but `true` saves nothing, and an answer after the call was given up is ignored); `update(preview)` saves the change to the same job and may return a `note`. A host without all three gets `update` refused with a message that tells the model not to delete and recreate the job instead. The tool's description says the same: in a live session, asked to change a job, a model deleted it and created it again, and its history went with it.
- `list` asks `host.list({ allFolders })`, `allFolders` being the model's (default `false`), and reports one line per job, `No scheduled jobs.` when there is none. A host read error becomes a tool refusal; the CLI host raises one with the unreadable-job count instead of claiming an empty list. A host may list every job regardless of `allFolders`; `ScheduleJobSummary.inSessionFolder` marks the session folder's, which the tool prints as `(this folder)`. The CLI's host does, because models do not pass `allFolders`: one was told "No scheduled jobs." right after creating a job in a folder below the session's.
