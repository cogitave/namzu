...
- The grant ends with the turn. `query()` creates one `SkillGrantSet` per turn, fills it from the `skill` tool and reads it in review. The next user message starts an empty set, and that includes a message the operator sends while the turn is still running. A host's `inboundMessages` (the TUI's queue for text typed mid-turn) and steering are delivered into the same `query()`, and the iteration clears the set (`SkillGrantSet.clear()`) when one of them arrives. The calls made for the new request are reviewed as usual unless the model loads the skill again.
...
- the call carries an escalation, meaning a sandbox escape or a path argument outside the working directory. Only a tool that declares its path argument (`pathArgument`) is checked for this. `bash` declares none, so what a command line does to paths is not an escalation. A `Bash(<pattern>)` entry therefore refuses redirection itself (see [Syntax](#syntax)), and a whole-tool `Bash` entry grants whatever a command line does, writes outside the directory included.
...
Any other name is looked up as written, without regard to case, so `mcp` tool names and host tools work. The `skill` tool reports a name the turn has no tool for as ignored. So is a registered tool that `allowedTools` withholds from the turn or the current step ("is not available in this turn"), because the model cannot call it, and that entry grants nothing. It never widens to everything. `NotebookEdit` and `TaskGet` have no tool here and are ignored.
...
**Patterns.** `Bash(<pattern>)` grants only command lines that match. The glob is the one the CLI's `[permissions]` table uses (`permissionPatternToRegExpSource`): `*` is any run of characters and `?` is one character, and a trailing ` *` also matches the bare command. `Bash(git status *)` therefore covers `git status` and `git status -s` but not `git statusx` or `git push`. A line is read as the commands it runs, and every one of them must match, so `git status && git push` is not covered. A line the reader cannot see through, such as a substitution, a heredoc or an ANSI-C quote (`$'…'`, whose escapes decode only when bash runs it), is not covered either. A pattern also never covers a line that redirects output into a file (`>`, `>>`, `>|`, `&>`, `&>>`, `<>`, `>&file`): `Bash(git status *)` covers `git status -s` and does not cover `git status > ~/.bashrc`, `git status -s >> /etc/passwd` or `git status > src/index.ts`. A pattern names a command, and where its output lands is not part of that command. The operator's `[permissions]` allow patterns come from the operator and do not make this exception, while a skill comes from the repository. `/dev/null` (`2>/dev/null`) and descriptor duplication (`2>&1`, `>&2`) write nothing and stay covered. A target the reader cannot resolve, such as `> "$OUT"`, counts as a write. The legacy form `Bash(npm run test:*)` means `npm run test *`. `Bash(*)` and `Bash` both grant the whole tool.
