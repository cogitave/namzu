...
A message may instead carry an opaque stored attachment ref. The store travels
with the invocation through `runAgent` and the model-running agent classes;
`query()` materializes the complete batch before creating provider work. One
finite phase deadline bounds even a custom store that ignores cancellation,
while a caller abort retains its own reason and the SDK never mutates the
caller's controller. Missing, mismatched, timed-out and partially resolved
attachments refuse rather than degrade. See [Stored attachment
resolution](runtime/stored-attachment-resolution.md).
...
An automatic pass publishes its context edit before the next provider request. After a sliding-window reduction, a structured summary, or a sufficient stale-tool-result clear, the existing `token_usage_updated` event carries unchanged cumulative spend alongside the estimated post-edit context and its exact window provenance. A host can therefore move its context indicator immediately; it need not wait behind a slow or failing request for a measurement of state that has already changed. An insufficient tool-result clear is staged rather than installed before LLM verification. If verification fails or is cancelled, the live history is untouched; if it succeeds, the clear and summary become one coherent history before their ordered events and final status snapshot are emitted.
...
`buildMemoryTools(store)` keeps retrieval on the store's asynchronous read
boundary. That distinction matters for a lazy disk store: its first
`search_memory` call loads the durable index before answering rather than
searching the fresh process's still-empty in-memory projection. The existing
`buildMemoryTools(store, index)` form is deliberately different: it treats the
caller-supplied index as already ready and authoritative, and never adds a
...
messages and tool call/result structure exact. See [Provider request
rich-content budgets](runtime/request-rich-content-budget.md) for the default,
configuration, compatibility opt-out, and persistence boundary.
