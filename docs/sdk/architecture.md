...
`CompactNowInput` and `CompactionResult` are exported alongside the host-triggered functions — a function on the public surface whose parameter and return types are not on it forces its first caller to inline both shapes, which is exactly what happened before they were.
