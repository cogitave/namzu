...
`mcpToolToToolDefinition` and `mcpPromptToToolDefinition` (`packages/sdk/src/connector/mcp/adapter.ts`, `prompt-adapter.ts`) build the pieces this function wraps and renames. They remain exported for compatibility with direct callers while the plugin path is migrated to `mcpToolset`; the CLI already uses the toolset path.
