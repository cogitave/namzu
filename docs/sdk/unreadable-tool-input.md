...
- A fragment with neither an index nor an id is placed only when exactly one
  open call — one already seen, whether named yet or not — could still be
  receiving more of its arguments right now: its buffer empty, or not yet a
  complete JSON value. A buffer that already parses as one complete value
  cannot take more, so that call is never a candidate; **an empty buffer can
  — it is not treated as already complete just because it has nothing in it
  yet**, which is what a call opens with, on the ordinary wire shape, before
  any of its argument text streams. Asked fresh for every such fragment, not
  decided once when some call opened: whether a call can still accept more
  changes over the stream, and only the state right now says which calls
  could be this fragment's target.
- When more than one open call could still accept the fragment — the
  ordinary shape is two calls opening back to back, both still empty, before
  either streams any argument text — or when none could, there is no way to
  tell which one it was for, or none was; guessing, as an id-less fragment
  otherwise does, can splice one call's JSON into the other's buffer, and
  the splice can still happen to parse, so the wrong call runs with no error
  at all. Every call that could have accepted the fragment is instead
  reported unreadable — `reason: 'malformed'`, since nothing here says the
  response was cut off, and "the model moved on to another call" is what
  opening a second call means — and the fragment itself is attributed to
  none of them. This does not refuse the stream: unlike a reused index,
  every such call is still answered, just as unreadable, and any other call
  keeps running. The accepted cost: a zero-argument call immediately
  followed by another call whose own arguments arrive in id-less fragments,
  from a server that also omits `index`, now comes out unreadable — the
  model is asked to send it again — rather than guessed at.
