...
declare function mintOrchestratorBearer(): Promise<string>
declare function fetchAllowlistForTenant(t: string): Promise<string[]>
...
const proxy = await new EgressProxy({
  ...
  onDenied: (host, reason) => console.warn({ host, reason }, 'egress denied'),
}).listen()
