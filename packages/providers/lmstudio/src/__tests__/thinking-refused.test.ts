...
const provider = new LMStudioProvider({ host: 'http://127.0.0.1:9', model: 'm' } as never)
...
describe('LMStudioProvider refuses a thinking request rather than dropping it', () => {
	it('throws on a manual thinking request', async () => {
		await expect(run({ type: 'enabled', budgetTokens: 10_000 })).rejects.toThrow(
			/LMStudioProvider does not implement thinking/,
		)
	})
