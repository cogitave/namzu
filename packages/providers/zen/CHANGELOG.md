...
- 54429af: `muse-spark-1.3` no longer advertises the `max` reasoning effort.
  ...
  driver has always given for an unadvertised level — _"The selected model does not
  advertise this reasoning effort level."_ Pass a lower level for this model, or use
  a model whose menu still carries `max`.

  **Why `minor` rather than `major`.** The catalogue's own rule, in
  [Zen and Zen Go](../../docs/sdk/zen.md), is that a removal is `major` _because a
  carried id stops resolving_; an addition or a repricing is `minor`. This model
