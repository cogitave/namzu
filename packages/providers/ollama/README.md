...
title: "@namzu/ollama"
description: >-
  The Ollama driver for @namzu/sdk. Wraps the official ollama client and
  implements the LLMProvider contract against a daemon you run yourself:
  streamed text, a request deadline, and the kernel's error taxonomy. Separate
  from the kernel so a consumer who runs no local model installs no client.
tags: [readme, package, provider, ollama, local-models]
...
**The Ollama driver for [`@namzu/sdk`](https://www.npmjs.com/package/@namzu/sdk).**
...
[![npm](https://img.shields.io/npm/v/@namzu/ollama.svg?label=%40namzu%2Follama)](https://www.npmjs.com/package/@namzu/ollama)

[Install](#install) · [Use it](#use-it) · [Configuration](#configuration) · [Capabilities](#capabilities) · [What it refuses](#what-it-refuses) · [Errors](#errors-and-cancellation) · [Health](#health-and-model-listing)
...
`@namzu/sdk` is a **peer** dependency (`>=1.3.0`), so your lockfile owns the
kernel version and one copy is shared with every other driver. The `ollama`
client (`^0.5.0`) is an ordinary dependency and arrives with this package.

You also need the daemon itself, which this package does not install. Get it
from [ollama.com](https://ollama.com), pull a model, and leave it running:
...
The driver talks to `http://localhost:11434` unless you tell it otherwise.
