...
const provider = new OllamaProvider({ host: 'http://127.0.0.1:9', model: 'm' } as never)
...
describe('OllamaProvider refuses a thinking request rather than dropping it', () => {
	it('throws on a manual thinking request', async () => {
		await expect(run({ type: 'enabled', budgetTokens: 10_000 })).rejects.toThrow(
			/OllamaProvider does not implement thinking/,
		)
	})
