 ...
 *   - Ownership is not per-tenant; only per-`SessionId`. There is no tenant
...
describe('EditOwnershipTracker', () => {
	...
	describe('claim', () => {
		it('claims an unowned file, emits ownership_claimed, returns ownership', () => {
			...
			const result = tracker.claim('/tmp/file.txt', sessionId(1))
			const after = Date.now()

			expect(result.claimed).toBe(true)
			if (result.claimed) {
				expect(result.ownership.owner).toBe(sessionId(1))
			...
			expect(events).toEqual([
				{ type: 'ownership_claimed', filePath: path.resolve('/tmp/file.txt'), owner: sessionId(1) },
			])
		})

		it('is idempotent when the same owner re-claims — no re-emit', () => {
			tracker.claim('/tmp/file.txt', sessionId(1))
			events.length = 0
		...
		it('denies a claim by a different owner, emits ownership_denied', () => {
			...
			const result = tracker.claim('/tmp/file.txt', sessionId(2))
			expect(result.claimed).toBe(false)
			if (!result.claimed) {
				expect(result.currentOwner).toBe(sessionId(1))
			...
			expect(events).toEqual([
				{
					...
					requester: sessionId(2),
					currentOwner: sessionId(1),
				},
			])
		})

		it('normalises file paths — equivalent paths collide', () => {
			tracker.claim('/tmp/foo/../file.txt', sessionId(1))
			const result = tracker.claim('/tmp/file.txt', sessionId(2))
			expect(result.claimed).toBe(false)
		})
	})
	...
	describe('release', () => {
		it('releases an owned file, emits ownership_released, returns true', () => {
			...
			const ok = tracker.release('/tmp/file.txt', sessionId(1))
			expect(ok).toBe(true)
			...
			expect(events).toEqual([
				{
					...
					filePath: path.resolve('/tmp/file.txt'),
					previousOwner: sessionId(1),
				},
			])
		})

		it('returns false when no ownership exists — no emit', () => {
			const ok = tracker.release('/tmp/never-claimed.txt', sessionId(1))
		...
		it('returns false when owner mismatches — no emit', () => {
			...
			const ok = tracker.release('/tmp/file.txt', sessionId(2))
			expect(ok).toBe(false)
			expect(tracker.getOwner('/tmp/file.txt')).toBe(sessionId(1))
			expect(events).toEqual([])
		})
	})
	...
	describe('transfer', () => {
		it('transfers ownership from current owner to another — atomic, single event', () => {
			...
			const ok = tracker.transfer('/tmp/file.txt', sessionId(1), sessionId(2))
			expect(ok).toBe(true)
			expect(tracker.getOwner('/tmp/file.txt')).toBe(sessionId(2))
			expect(events).toEqual([
				{
		...
		it('returns false when the `from` argument is not the current owner — no emit', () => {
			...
			const ok = tracker.transfer('/tmp/file.txt', sessionId(99), sessionId(2))
			expect(ok).toBe(false)
			expect(tracker.getOwner('/tmp/file.txt')).toBe(sessionId(1))
			expect(events).toEqual([])
		})
		...
		it('refreshes claimedAt on successful transfer', async () => {
			...
			expect(t0).toBe(sessionId(1))

			await new Promise((r) => setTimeout(r, 2))
			tracker.transfer('/tmp/file.txt', sessionId(1), sessionId(2))
			const list = tracker.listByOwner(sessionId(2))
			expect(list).toHaveLength(1)
			const after = list[0]
			expect(after?.claimedAt).toBeGreaterThan(0)
			expect(after?.owner).toBe(sessionId(2))
		})
	})

	describe('releaseAll', () => {
		it('releases every ownership for the owner, returns count, emits per-entry', () => {
			tracker.claim('/tmp/a.txt', sessionId(1))
			tracker.claim('/tmp/b.txt', sessionId(1))
			tracker.claim('/tmp/c.txt', sessionId(2))
			events.length = 0

			const count = tracker.releaseAll(sessionId(1))
			expect(count).toBe(2)
			expect(tracker.getOwner('/tmp/a.txt')).toBeUndefined()
			expect(tracker.getOwner('/tmp/b.txt')).toBeUndefined()
			expect(tracker.getOwner('/tmp/c.txt')).toBe(sessionId(2))

			const released = events.filter((e) => e.type === 'ownership_released')
			expect(released.length).toBe(2)
			for (const e of released) {
				if (e.type === 'ownership_released') {
					expect(e.previousOwner).toBe(sessionId(1))
				}
			}
		})

		it('returns 0 when the owner has no entries', () => {
			const count = tracker.releaseAll(sessionId(99))
			expect(count).toBe(0)
			expect(events).toEqual([])
		})
	})
	...
	describe('read helpers', () => {
		...
		it('listByOwner returns all ownerships for a given owner', () => {
			...
			const list = tracker.listByOwner(sessionId(1))
		...
		it('checkConflict returns the current owner iff a different owner holds the file', () => {
			tracker.claim('/tmp/file.txt', sessionId(1))
			expect(tracker.checkConflict('/tmp/file.txt', sessionId(1))).toBeUndefined()
			expect(tracker.checkConflict('/tmp/file.txt', sessionId(2))).toBe(sessionId(1))
			expect(tracker.checkConflict('/tmp/other.txt', sessionId(2))).toBeUndefined()
		})
	})
})
