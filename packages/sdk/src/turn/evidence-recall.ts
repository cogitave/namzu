...
import type { SessionEvidenceScope, SessionTextEvidenceSource } from '../store/evidence/types.js'
import type { Message } from '../types/message/index.js'
import type { PrepareStep, PrepareStepContext } from '../types/session/prepare-step.js'
...
export interface EvidenceRecallRequest {
	/** The session the recall is for; the host binds retrieval to it. */
...
function address(candidate: ScopedEvidenceRecallCandidate) {
	return {
		...(candidate.scope.turnId !== undefined ? { turnId: candidate.scope.turnId } : {}),
		seq: candidate.seq,
		part: candidate.part,
		...
		validate(candidate) {
			if (
				Object.entries(scope).some(
					([key, value]) => candidate.scope[key as keyof SessionEvidenceScope] !== value,
				) ||
		...
		try {
			const operation = Promise.resolve().then(() =>
				retrieve({
					...
					...(captureSessionEvidence
						? {
								captureSessionEvidence: async (maxReadBytes?: number) => {
									controller.signal.throwIfAborted()
									const source = await captureSessionEvidence(maxReadBytes, controller.signal)
									controller.signal.throwIfAborted()
									return source
								},
							}
			...
			for (const candidate of batch.candidates) {
				if (
					...
					(candidate.scope.turnId !== undefined && !isEntityId(candidate.scope.turnId, 'turn'))
				)
