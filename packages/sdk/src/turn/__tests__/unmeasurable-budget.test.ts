 ...
 * tokens something had a rate for. So a turn partly (or wholly) priced at no
 * rate sits under any limit forever while spending without bound — which was
 * the state of every turn before the price catalogue, since nothing fed the
 * calculation at all.
 *
...
describe('costLimitUsd against a partly unpriced run', () => {
	...
	it('reports the exceeded budget first when the turn is both', () => {
	...
	it('does not fire when no cost limit was set', () => {
		...
		// stop every turn on every uncatalogued model, which is a far larger
