...
export interface JsonClaimVerifier {
	...
	verify(
		claims: unknown,
		context: Pick<AnswerReviewContext, 'turnId' | 'iteration' | 'signal'>,
...
export function createJsonClaimVerifier(options: JsonClaimVerifierOptions): JsonClaimVerifier {
	const { scope, turnId, observe } = options
	...
	return {
		...
		async verify(claims, context) {
			...
			try {
				...
				return {
					accept: true,
					receipt: { scope, turnId, iteration: context.iteration, claims: values, observations },
				}
			} catch {
				context.signal?.throwIfAborted()
				return reject(
