...
> {
	...
	/**
	 * A shell of this agent that one turn may have to itself.
	 *
	 * An agent instance holds per-turn state — an abort controller, the id of
	 * the turn in flight — and refuses a second concurrent `run` call because
	 * of it. That refusal is correct for a host calling `run` twice on purpose:
	 * two overlapping turns would share one abort controller, so cancelling
	 * either kills both.
	 *
	 ...
	 * `agent_id` four times drove four turns at one shell: one worked and three
