...
describe('keys the skill loader does not read', () => {
	...
	it('do not relax a key the loader reads', async () => {
		// `allowed-tools` as a list is still refused: a capability that is read
		// wrongly is the failure the refusal exists for.
		const path = skill(
			['---', 'name: a-skill', 'description: d', 'allowed-tools: [Read, Grep]', '---', 'b'].join(
				'\n',
			),
		)
		await expect(loadSkill(path, 'full')).rejects.toThrow(/flow sequence/)
