...
/**
 * A turn has to survive its own waiting.
 *
 * Every HITL park went through a timer that was deliberately `unref`'d, so a
 * pending park-recorder could never hold a process open after the turn
 * settled. The intent was right and the scope was wrong: the turn AWAITS that
 * timer, mid-turn, on every park. An unref'd timer does not keep Node's loop
 * alive — so once the decision resolved and the turn sat out the rest of the
...
/**
 * A scripted provider and a turn, in a file with no test runner under it.
 *
...
describe('a turn outlives its own HITL park', () => {
	it('finishes in a process with nothing else holding the event loop open', () => {
		...
		// The turn reached its own end rather than the process reaching it first.
		expect(out, `the turn did not complete:\n${out}`).toContain('RESULT ')
