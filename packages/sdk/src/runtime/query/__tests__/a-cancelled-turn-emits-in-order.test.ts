...
/**
 * A cancelled turn is the one path where ORDER is the entire contract.
 *
 * A host folds `message_completed` into its transcript and `turn_completed`
 * into its turn record. A stream that settles the turn before it closes the
 ...
 * dozen separate points and each one settles the turn by hand.
 */
...
const baseParams = async (overrides: Partial<QueryParams>): Promise<QueryParams> =>
	({
		...
		// What `drainQuery` substitutes. `query()` does not, and a turn with no
...
describe('a turn cancelled while the provider held the turn', () => {
	it('closes the message before it settles the turn, and never reports a failure', async () => {
		...
		// The message must be closed BEFORE the turn is settled, or a host that
...
describe('a turn cancelled between one iteration and the next', () => {
	it('settles once, after the iteration it interrupted has reported', async () => {
		...
		const { events, run } = await drain(query(params), (event) => {
			...
			// which is the boundary a cancelled turn crosses.
		...
		// And the turn did not go round again — the turn after the abort is the
...
describe('a Stop that arrives while the turn is parked on a tool review', () => {
	...
	it('resolves the park as cancelled rather than waiting for the host to answer', async () => {
		...
		const params = await baseParams({
			...
			resumeHandler: () => {
				...
				// cancellable this turn would hang here until the test timed out.
				return new Promise(() => {})
			},
	...
	it('leaves the durable park it recorded resolved, not outstanding', async () => {
		...
		// turn-level claim is that the park is on the durable record at all.
...
describe('the in-iteration abort checkpoints', () => {
	 ...
	 * iteration and each one settles the turn by hand. Two of them settling —
	 * or one of them failing to — is invisible in a single-event assertion, so
	 * the invariant is asserted at every point a real turn can be reached.
	 */
	...
	for (const { name, abortAfter } of cases) {
		it(`settles the turn exactly once when the Stop lands after ${name}`, async () => {
			const caller = new AbortController()
			const provider = new MockLLMProvider({
				turns: [
					{ toolCalls: [{ id: 'c1', name: 'noop', args: {} }], finishReason: 'tool_calls' },
...
describe('why a turn was cancelled', () => {
	...
	for (const cause of nonUserCauses) {
		it(`records cancelCause: '${cause}' and leaves the user-interrupt hooks alone`, async () => {
			...
			// host's cleanup against a turn nobody stopped.
			expect(seen).toEqual([])
		})
	}
