...
async function mkWorkdir(): Promise<string> {
	...
	// `os.tmpdir()` is itself a symlink on macOS. A turn whose working directory
	// is a link is a turn whose ledger nothing here would find.
	return realpath(dir)
}

/**
 * A provider and registry that make the turn actually ITERATE.
 *
...
describe('a resumed turn remembers the files this conversation wrote', () => {
	const written = 'alpha\nbeta\n'
	...
	it("carries the interrupted turn's own write instead of the body it replaced", async () => {
		...
		// write inside it that DID land — its receipt recovered from the turn's
