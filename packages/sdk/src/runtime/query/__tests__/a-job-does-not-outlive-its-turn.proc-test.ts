...
/**
 * A background job outlives its tool call. It must not outlive its turn.
 *
 * Outliving the call is the whole point, which is exactly what makes an
 * unterminated one an orphan: the turn ends, the ids go with it, and the
...
describe('a turn takes its background jobs with it', () => {
	it('leaves nothing of its own running', async () => {
		const registry = new BackgroundJobRegistry()

		const run = (await runStartingAJob(registry, 'sleep 30')) as Turn

		// Nothing is left under this turn's id — and the id is real, so a
	...
	it('reaches what the job itself forked, not only the shell', async () => {
		...
		// Without this synchronization the turn can finish before the spawned
		...
		if (!job) throw new Error('the turn started no job')
	...
	it('kills them on a FAILED run too', async () => {
		// The `finally`, not the happy path. A turn that threw has the same
		...
		// The turn must actually have failed, or this asserts the happy path
	...
	it('leaves another owner’s jobs alone', async () => {
		// Teardown is scoped to the turn's id. A shared registry serving
		// several turns must not have one of them tear down another's work.
		const registry = new BackgroundJobRegistry()
