...
/**
 * What a run does when the model stops calling tools while a job it said it
 * was waiting on is still running.
 *
 * Before this, nothing: the run settled, and the exit notice — which needs a
...
describe('a run suspends for a job the model awaited, and pays no tokens for it', () => {
	...
	it('goes on waiting for a second job after the first one’s exit was read', async () => {
		...
		// before it began, the drain found no notice to deliver, and the run
		...
		// would mean the run settled at the text turn instead of waiting.
		expect(provider.requests.length).toBe(6)
		expect(
			run.abandonedJobIds,
			'the run walked away from the job it was waiting on',
		).toBeUndefined()
	...
	it('leaves a job nobody awaited to run, and ends the run at once', async () => {
		// The dev-server case. `sleep 30` outlives this run's twenty-second
		// budget, and a run that held for it would sit here for about nine
		...
		expect(run.status).toBe('completed')
		expect(Date.now() - startedAt, 'the run held open for a job nobody awaited').toBeLessThan(3_000)
