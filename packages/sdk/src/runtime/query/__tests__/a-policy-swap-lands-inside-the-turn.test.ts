...
/**
 * The swap has to land INSIDE the turn, or none of it matters.
 *
...
describe('a host can reach the turn’s policy box', () => {
	...
	it('names an unattended turn `auto-approve`, not `host`', async () => {
		...
		// name every turn `host`, including the ones approving everything
		// unattended.
		let seen: SessionApprovalPolicy | undefined
		await runWithPolicy({
			onPolicy: (policy) => {
	...
	it('names a turn with a real handler `host` by default', async () => {
		let seen: SessionApprovalPolicy | undefined
		await runWithPolicy({
			onPolicy: (policy) => {
				seen = policy
...
describe('a change made during the turn reaches the turn’s event stream', () => {
	it('emits approval_policy_changed to the turn’s listener', async () => {
...
describe('the swap reaches the places that actually ask a human', () => {
	it('the SECOND tool review is answered by the NEW handler', async () => {
		...
		// The shape, not an exact count: how many reviews a turn raises is the
...
describe('the swap reaches PLAN approval too, which is the other place a human is asked', () => {
	...
	it('a plan raised after the swap is answered by the new handler', async () => {
		...
		// Both objects outlive the turn, and the question this asks is about
...
describe('the model is told, in the slot it already reads', () => {
	it('carries the notice on the request AFTER the swap, and only that one', async () => {
		// The model plans around how closely it is watched. A turn that
	...
	it('says nothing to a turn whose policy never moved', async () => {
