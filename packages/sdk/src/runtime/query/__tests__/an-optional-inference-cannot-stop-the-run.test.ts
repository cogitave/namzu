...
/**
 * A run's optional inference is bounded by the run, not by itself.
 *
 ...
 * 17.3 s, the deadline fired at 10.0 s, and the run stopped 10.0 s after the
 ...
 * are the provider's own request timeout and the run's cancellation, which is
 * exactly what every other model request in the run has.
 */
...
it('completes the run when the optional inference outlives the deadline it was given', async () => {
	...
	const result = await run(async ({ generateText }) => {
		...
		// shared ledger unresolved and stopped the run.
		await generateText!({ ...input, timeoutMs: 10 })
		return undefined
	}, provider)

	// The main model was asked. This is the assertion the defect failed: the run
