...
/**
 * Every turn reported a cost of `$0.00`.
 *
 * `calculateCost` existed, `CostInfo` was carried on the turn, the step, the
 ...
 * proves nothing about whether a turn ever reaches it. Before the price
...
describe('a turn against a catalogued model', () => {
	...
	it('prices from the model actually asked for, not from the turn alone', async () => {
	...
	it('prices a turn at the rate of the member that SERVED it, not the one declared', async () => {
		// Every other case here runs a single provider on the turn's own model,
	...
	it('reports a genuinely free run as free, not as unknown', async () => {
		...
		// The distinction. A local turn costs nothing and we know it; the case
		// below costs something and we do not.
		expect(settled.costInfo.unpricedTokens).toBe(0)
	})
})

describe('a turn against a model nobody has a rate for', () => {
	it('reports unknown, distinctly from free', async () => {
		...
		// `totalCost === 0` passes against the defect being fixed; the two turns
...
describe('costLimitUsd when the cost cannot be measured', () => {
	it('refuses the turn up front rather than running it unbudgeted', async () => {
	...
	it('does not refuse a turn that set no limit', async () => {
		// An unpriced model is only a problem for a budget. A turn without one
