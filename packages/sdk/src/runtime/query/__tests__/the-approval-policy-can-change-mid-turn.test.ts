...
/**
 * Who answers when the turn asks a human, as a value rather than a closure.
 *
 ...
 * ahead, I'm stepping out" meant ending the turn — discarding the in-flight
...
describe('the model is told once, and only about the state it is now under', () => {
	...
	it('starts reporting again after a later change', async () => {
		// Cleared, not disabled. A turn whose policy moves twice with a model
