 ...
 * that set, so a turn resumed with `{action: 'approve_plan'}` COMPLETED while
 * its park stayed outstanding.
 *
 * The harm is not cosmetic, and it is the same harm twice over: a finished turn
 * keeps being served by `findPendingCheckpoint`, so a host that asks again is
 * told a human still owes this turn an answer and a second resume is refused
