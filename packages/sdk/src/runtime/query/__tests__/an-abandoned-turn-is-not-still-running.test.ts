...
/**
 * A consumer that walks away leaves a turn that is over, and the durable
 ...
 * everything after it. The turn is torn down and the log holds no terminal
...
interface TurnUnderTest {
	...
	/** The teardown the turn's `finally` block performs. */
...
/**
 * Consume the turn until it is genuinely mid-flight, then break.
 *
 * Breaking at `checkpoint_created` is deliberate: by then the turn has
 * started, a checkpoint is committed and the log holds a running turn —
 * the state a host would find if it looked while the turn was working.
 */
async function abandonMidFlight(): Promise<TurnUnderTest> {
	...
	// `break` only reaches the `finally` if the turn actually got there.
	expect(sawTurnInFlight).toBe(true)
	return run
}

/** Drive a turn to its terminal value. A manual drain, because `for await`
...
describe('a consumer that abandons the turn', () => {
	it('runs the turn’s teardown', async () => {
		const run = await abandonMidFlight()

		expect(run.killOwner).toHaveBeenCalledTimes(1)
	})

	it('leaves a terminal record rather than one that says the turn is alive', async () => {
		const run = await abandonMidFlight()
	...
	it('leaves a turn that completes writing exactly the same number of times', async () => {
		// The abandonment handling must not add a write where the turn already
 ...
 * away: the turn is resumable and somebody is still owed an answer.
 *
 * The abandonment path must not write a verdict over that. `deriveTurnStatus`
 * reads a terminal status FIRST and the park second, so a parked turn recorded
...
describe('a consumer that walks away while a human is being asked', () => {
	it('leaves the turn parked, so the record still reads awaiting_hitl', async () => {
		const scope = {
			tenantId: generateTenantId(),
			projectId: generateProjectId(),
