 ...
 *
 * These drive `runAgent`, because the claim is about a run rather than about
 * the registry: the tool is the host's, the registry is the host's, and what
 * changes is what the run asks for.
 */
...
describe('a run screens a registry it did not build', () => {
	...
	it('leaves a host tool alone, because its answer may legitimately be the request', async () => {
		// The web_fetch case, at the run level: a page whose body IS the URL.
		const mock = provider()
