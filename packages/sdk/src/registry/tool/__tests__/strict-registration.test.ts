 ...
 * only once something actually ran. The manager already refuses
 * `enforceModelInput` without a `modelInputSchema`, with a comment stating the
 * principle exactly: "Refusing at admission puts the error where the author
...
function admit(def: ToolDefinition): () => ToolManager {
	return () => new ToolManager({ toolsets: [testToolset(def)], messages: () => [] })
}
...
describe('a tool cannot register a schema its own guarantee cannot carry', () => {
	...
	it('leaves a tool that never asked for the guarantee alone', () => {
		...
		expect(
			admit(tool({ modelInputSchema: { properties: { a: { oneOf: [{ type: 'string' }] } } } })),
		).not.toThrow()
	})
	...
	it('still refuses enforcement with no model schema at all', () => {
		...
		expect(admit(tool({ enforceModelInput: true }))).toThrow(/does not define modelInputSchema/)
	})
})
