...
function findExecPositions(words: readonly ShellWord[], dialect: ShellDialect): ProgramPosition[] {
	...
	for (let i = 0; i < words.length; i += 1) {
		...
		const clause = words.slice(clauseStart, k)
		if (clause.length === 0) {
			positions.push({ unknown: `find's ${w.value} clause has no program` })
		} else {
			// find execs the entire argv after -exec, including any re-exec
			// wrappers. Inspecting only its first word would mistake `env` for
			// the program and miss a dynamic name further into the clause.
			for (const position of resolveChain(clause, dialect)) {
				if (position.word && LITERAL(position.word) && position.word.value.includes('{}')) {
					positions.push({
			...
			// The outer lexer only sees `find`, so it cannot read a shell -c
			// payload inside this clause. Read it here with the same wrapper
			// and shell-option grammar the lexer uses for a top-level shell.
			const nested = nestedShellCommand(clause)
			if (nested !== null) {
				if ('opaque' in nested) {
					positions.push({ unknown: nested.opaque })
				} else if (nested.shell === 'zsh' || nested.shell === 'ksh' || nested.shell === 'mksh') {
					positions.push({ unknown: `nested ${nested.shell} is not modeled` })
				} else if (LITERAL(nested.payload) && nested.payload.value.includes('{}')) {
					positions.push({
						unknown: 'find substitutes `{}` into this shell payload before it runs',
					})
				} else {
					positions.push(
						...resolveDashCPayload(nested.payload, nested.shell === 'bash' ? 'bash' : 'sh'),
					)
				}
			}
		}
...
export function programPositions(
	...
	for (const position of positions.slice()) {
		if (!position.word || !LITERAL(position.word) || basename(position.word.value) !== 'find')
			continue
		const index = command.words.indexOf(position.word)
		if (index >= 0) positions.push(...findExecPositions(command.words.slice(index + 1), dialect))
	}
	return positions
}
