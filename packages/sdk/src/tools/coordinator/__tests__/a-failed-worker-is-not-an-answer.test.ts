 ...
 * gateway's `state` is `'completed'` for a child whose turn returned
...
describe('success needs both authorities to agree', () => {
	it('refuses a child the gateway called complete but whose turn failed', () => {
		// The exact shape the kernel produces: markCompleted was called, and
		// the turn underneath it did not succeed.
	...
	it('accepts a gateway that reports no turn status at all', () => {
		// A host gateway need not surface a turn status. Treating its absence as
...
describe('the failure is named by whichever layer reported it', () => {
	it('uses the task state when the task itself did not complete', () => {
		...
		// and a task whose turn errored call for different next moves.
		expect(failureLabel(handle('canceled', 'completed'))).toBe('canceled')
	})

	it('uses the turn status when the task completed but the turn did not', () => {
...
describe('create_task itself reaches the predicate', () => {
	...
	const settled = (status: string): TaskHandle =>
		({
			...
			agentId: 'reviewer',
			// The kernel's own shape: markCompleted ran regardless of the turn.
			state: 'completed',
			createdAt: 1_000,
			completedAt: 2_000,
	...
	it('reports a failed turn as a failure', async () => {
	...
	it('still reports a successful turn as a success', async () => {
