...
async function seed(store: SessionStore, tenantId: TenantId) {
...
export function sessionStoreContract(name: string, factory: () => Required<SessionStore>): void {
	describe(name, () => {
		...
		it('rejects cross-tenant project reads with TenantIsolationError', async () => {
			...
			await expect(store.getProject(project.id, tenantB)).rejects.toBeInstanceOf(
		...
		it('rejects cross-tenant session reads with TenantIsolationError', async () => {
			...
			await expect(store.getSession(session.id, tenantB)).rejects.toBeInstanceOf(
				TenantIsolationError,
			)
		})
		...
		it('rejects cross-tenant updateSession (payload tenant mismatch)', async () => {
			...
			await expect(store.updateSession(forged, tenantA)).rejects.toBeInstanceOf(
				TenantIsolationError,
			)
		})
