...
export function assertBudgetEnforceable(config: {
	...
	if (config.budget?.maxCostPerTurn === undefined) return

	const unpriced = config.advisors.filter((a) => a.pricing === undefined).map((a) => a.id)
	if (unpriced.length === 0) return

	throw new Error(
		`Advisory budget sets maxCostPerTurn but ${unpriced.length === 1 ? 'advisor' : 'advisors'} ` +
			...
			'entry, or drop the cost cap and bound the turn with `maxCallsPerTurn`.',
	)
}
