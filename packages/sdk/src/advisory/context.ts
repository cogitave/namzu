...
/**
 * What the turn looks like right now, for an advisory call that did not come
 * from the iteration loop.
 *
 * A function rather than a snapshot because the tool is built once per turn
 * and called at an unknown later point: capturing the context at
 * construction would hand every advisor the state the turn had before it
 * started.
 */
...
export class AdvisoryContext {
	...
	/** Wired by the runtime once the turn exists. */
	...
	getBudgetStatus(): { remaining: number | undefined; total: number | undefined; used: number } {
		...
		const total = this.budget?.maxCallsPerTurn
		const remaining = total !== undefined ? total - used : undefined
		return { remaining, total, used }
	}

	/** Advisory spend so far this turn, summed from the recorded calls. */
	spentCost(): number {
	...
	checkBudget(): { allowed: boolean; reason?: string } {
		...
		if (remaining !== undefined && remaining <= 0) {
			return {
				allowed: false,
				reason: `Advisory budget exhausted: ${total} calls used of ${total} allowed per turn`,
			}
		}

		// Checked before the call, not after: a cap that only reports
		// overspend once it has happened is a log line, not a budget.
		const costCap = this.budget?.maxCostPerTurn
		if (costCap !== undefined) {
			const spent = this.spentCost()
			if (spent >= costCap) {
				return {
					allowed: false,
					reason: `Advisory budget exhausted: cost ${spent} of ${costCap} allowed per turn`,
				}
			}
		}
