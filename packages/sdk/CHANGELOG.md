...
- 4e8cf5c: A tool result that is the request is now refused by default — scoped to results framed as untrusted, with an off switch and a per-tool exception
...
- 19abb2c: Nothing a consumer installs changes. The two new files are `*.test.ts`, which
...
- ad1bab9: Correct the quick start and package summary to distinguish host-supplied
  sandbox, durable stores and telemetry exporters from kernel defaults.
