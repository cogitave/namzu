...
describe.skipIf(!E2E)('the Windows engine from WSL', { timeout: 120_000 }, () => {
	...
	async function drive(host: PlaywrightBrowserHost) {
		...
		expect(back.message ?? '').not.toMatch(/no page to go back/)
		const forward = await host.observe({ action: 'forward' })
		expect(forward.page?.url).toBe(`${origin}/form.html`)
		expect(forward.message ?? '').not.toMatch(/no page to go forward/)
		const nowhere = await host.observe({ action: 'forward' })
		expect(nowhere.message).toMatch(/There is no page to go forward to/)
