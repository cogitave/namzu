...
describe('programs inside find exec clauses', () => {
	it('refuses a dynamically selected program behind a wrapper', () => {
		const policy = policyFor({ rules: { bash: { 'echo*': 'deny' } } })
		const body = 'find . -maxdepth 0 -exec env $(printf e)cho marker \\;'
		expect(ok(body, 'bash', policy)).toBe(false)
	})

	it('refuses an unreadable shell command inside the clause', () => {
		expect(ok('find . -maxdepth 0 -exec env bash -c "$X" \\;')).toBe(false)
	})

	it('keeps a literal wrapped program eligible', () => {
		expect(ok('find . -maxdepth 0 -exec env echo marker \\;')).toBe(true)
	})
})
