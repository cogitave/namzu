 ...
 *
 * The job's own `script.shell` was verified against a specific dialect at
 * confirm time (`verifyScheduledScript`). If the HOST's own resolved shell
 * no longer matches that dialect (bash was removed since the job was
 * confirmed, say), running it anyway would mean the rules' reading and the
 * shell that actually runs the line have quietly drifted apart — exactly
 * what `command-shell.ts` exists to prevent for a live turn. So a mismatch
 * is refused here, the same way a moved folder or a changed project config
 * is refused elsewhere in `runFire`, rather than silently downgraded or
 * upgraded to whatever the host happens to have today.
 */

import { type ExecHostShellProgress, execHostShell, hostCommandShell } from '@namzu/sdk'
...
export interface ScriptRunResult {
	...
	/** The host's shell no longer matches the dialect the script was verified in. */
...
export async function runScript(
	...
	if (host.dialect !== shell) {
		return {
			stdout: '',
			stderr: '',
			exitCode: null,
			timedOut: false,
			dialectMismatch: { expected: shell, actual: host.dialect },
		}
	}
	try {
