...
describe('/schedule confirm for a script job', () => {
	it('shows control and bidi characters as code points without changing the confirmed script', async () => {
		...
		await runScheduleCommand(['confirm', original.name], {
			...
			say: (text) => said.push(text),
			ask: async () => {
				const preview = said.join('\n')
				expect(preview).toContain('\\u{001b}[2J\\u{202e}')
	...
	it('projects hidden characters in a persisted folder path on the job list', async () => {
		const original = confirmedJob(sb, { name: 'listed' })
		const folder = `${sb.project}\u001b[2J\u202e`
		updateJob(sb.paths, original.id, original.revision, (job) => ({
			...job,
		...
		const listed = await listScheduleJobs({
			...
			config: {},
			say: () => {},
			ask: async () => ({ kind: 'answer', selectedOptionIds: [] }),
		})
		expect(listed).toContain('\\u{001b}[2J\\u{202e}')
...
describe('the schedule tool’s host', () => {
	...
	describe('a model-proposed script/script+agent job', () => {
		...
		it('projects controls and bidi in model-proposed script text before a TUI answer', async () => {
			...
			expect(result.success).toBe(true)
			expect(said[0]).toContain('\\u{001b}[2J\\u{202e}')
			expect(said[0]).not.toContain('\u001b')
			expect(said[0]).not.toContain('\u202e')
			expect(listJobs(sb.paths).jobs[0]?.script?.body).toBe(body)
		})
