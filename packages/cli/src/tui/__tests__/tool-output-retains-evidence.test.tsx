...
const OUTPUT = [
	LONG_LINE,
	...Array.from({ length: 239 }, (_, i) => `evidence-${i}`),
	'FINAL_DIAGNOSTIC',
].join('\n')
...
it.each(['\u000f', '/raw'])(
	'recovers output beyond 200 lines through the App with %s',
	async (command) => {
		...
		expect(text()).not.toContain('FINAL_DIAGNOSTIC')
		screen.press(command)
		await screen.waitForRender()
		if (command.startsWith('/')) screen.press('\r')
		await until(screen, () => text().includes('FINAL_DIAGNOSTIC'))
