...
import type { Message, StopReason, StructuredOutputConfig } from '@namzu/sdk'
...
import { execJson } from './exec-json.js'
import { duration, pauseWait } from './provider-wait.js'
import { resolveResume } from './resume.js'
import { readStdin } from './stdin.js'
import type { CommandDef, CommandHandlerArgs } from './types.js'
...
/**
 * What `exec` prints on stderr when a termination signal stops it, worded from
...
/**
 * `--output-schema <file>`, loaded, or the reason it could not be. Resolved
 * against the process's directory, like every other path on the command line.
 */
function loadExecOutputSchema(
	path: string | null,
): StructuredOutputConfig | { readonly error: string } | undefined {
	if (path === null) return undefined
	const absolute = resolve(process.cwd(), path)
	try {
		return loadOutputSchema(absolute)
	} catch (error) {
		return {
			error: `--output-schema ${absolute}: ${error instanceof Error ? error.message : String(error)}`,
		}
	}
}

/** The default mode: print the reply. */
async function execPrint(
	...
	if (structuredOutput && 'error' in structuredOutput) {
		ctx.formatter.error({ message: structuredOutput.error })
		return EXIT_USAGE
	}
	// `--session` keys a host's conversation for `--json`. This mode resumes
	// with --continue or --resume; accepting the key and ignoring it would
	// answer against no history while looking like it answered against one.
	if (flags.session !== null) {
		ctx.formatter.error({
			message:
				'--session binds a host conversation for `exec --json`; this mode resumes with --continue or --resume <id>',
		})
		return EXIT_USAGE
	}
	...
	// namzu exec` and for the `-` sentinel. Alongside a prompt it is extra
	...
	const modeResult = resolvePermissionMode({
		...
		// A headless turn has nobody to ask, so `prompt` here would silently
	...
	try {
		sessionId =
			resume.kind === 'resumed' ? asSessionId(resume.sessionId) : await startConversation(sessions)
	} catch (error) {
	...
	// the TUI. A scripted turn under an accepted capability limitation is
	...
	// turn is bound by. A script that reads AGENTS.md aloud is not a script
	...
	// server is a child process, and an `exec` that returns without closing
	// leaves it behind.
	await session.close()
	closeSessions(sessions)
	...
	// `namzu exec … > out.txt && deploy` proceeded on the empty file.
...
export const execCommand: CommandDef = {
	name: 'exec',
	aliases: ['e'],
	description: 'Run a single prompt headlessly: print the reply, or stream events with --json',
	passThrough: true,
	help: [
		'Usage: namzu exec [options] <prompt...>',
		'       echo "<prompt>" | namzu exec',
		'       cat file.txt | namzu exec "summarise this"',
		'       namzu exec --json [--session <id>] <prompt...>',
		'',
		'Run a single prompt headlessly. Everything that is not an option is the',
		'prompt. `namzu e` is the same command.',
		'',
		'By default the reply is printed on stdout and status lines go to stderr.',
		...
		'is about. `namzu exec -` reads the prompt from stdin explicitly.',
		'',
		'Options:',
		'  --json                Stream one JSON event per line instead of the reply',
		'  --output-schema <f>   Bind the final answer to the JSON Schema in file f,',
		"                        using the provider's native structured output",
		...
		'  --session <id>        With --json: bind the turn to the host conversation',
		'                        keyed by this id',
		...
		'and say why. Neither ever falls back to starting a new one: leave both',
		'out if that is what you want.',
		'',
		'Needs a provider. Set a credential in the environment, or run namzu',
		'once to pick one interactively.',
		'',
		'A turn the provider pauses keeps a checkpoint. Without --wait-for-provider',
		'it exits 75 at once and a wrapper decides when to try again; with it, the',
		"turn waits the provider's own delay when it named one (otherwise a minute,",
		...
		'wait, then try again — or when the conversation already has an active',
		...
		'With --json:',
		'',
		'Each line is one event as the turn unfolds — text deltas, tool starts and',
		'ends, usage, then a terminal done event. done.text is the settled answer;',
		'earlier deltas may include progress or answers later rejected by',
		'verification. History is bound with --session <id>; --continue, --resume',
		'and --wait-for-provider are refused. Without --session, optional stdin',
		'must be one complete JSON Message[]; invalid or provider-incomplete tool',
		'history is refused before a turn.',
		'',
	...
	handler: withTerminationHandling(async (args, termination) => {
		const flags = parseExecFlags(args.rawArgs)
		const structuredOutput = loadExecOutputSchema(flags.outputSchema)
		return flags.json
			? execJson(args, termination, flags, structuredOutput)
			: execPrint(args, termination, flags, structuredOutput)
	}),
}
